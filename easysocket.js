var easysocket = {
    _prepareSocket: function(socket){
		//console.log('_prepareSocket',socket.chunck);
        socket.chunck = socket.chunck || {
            messageSize : 0,
            buffer: new Buffer(0),
    ...
    recieve: function(socket, data,callback) {
        ...
        do {
            ...
            if (socket.chunck.messageSize == 0 && socket.chunck.bufferStack.length >= 4) {
				...
				//console.log('socket.chunck.messageSize',socket.chunck.messageSize);
            }

            if (socket.chunck.messageSize != 0 && socket.chunck.bufferStack.length >= socket.chunck.messageSize + 4) {
				//console.log('socket.chunck.bufferStack',socket.chunck.messageSize,socket.chunck.bufferStack.length);
