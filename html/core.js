...
function MainCtrl($scope,$http,$mdBottomSheet, $mdSidenav,$mdDialog,AppFactory) 
{
	...
	self.SaveMap = function(){
		var product = AppFactory.GetPoductMap();
		var payload = new FormData();
		if(product.map){
			//delete product.map.id;
			product.map.id = self.mapName.product_id;
			payload.append("map", JSON.stringify(product.map));
		}
		
		payload.append("booking", JSON.stringify(product.booking));
		$http({
			method: 'POST',
			url: 'http://m8s.nsupdate.info/lucky/theatre?mode=save&name='+self.mapName.name+'&product_id='+self.mapName.product_id,
			data: payload,
